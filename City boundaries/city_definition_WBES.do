...
******Prepare to join with ArcGIS city features (match ID: city polygon FID)********
*Note: when exporting attribute tables from ArcGIS, select: open attribute table - table options - export txt - change the format to csv. That avoids messing up city names.
import delimited D:\Research\WEBS\Geo_data\match1_simp.csv, varnames(1) encoding(UTF-8) clear
drop if idstd==.
keep join_fid idstd
drop if idstd==532224 & join_fid ==14241
save "D:\Research\WEBS\Geo_data\match1_simp.dta", replace

import delimited D:\Research\WEBS\Geo_data\match2_simp.csv, varnames(1) encoding(UTF-8) clear
drop if idstd==.
keep join_fid idstd
save "D:\Research\WEBS\Geo_data\match2_simp.dta", replace

use "D:\Research\WEBS\Geo_data\match1_simp.dta", clear
append using "D:\Research\WEBS\Geo_data\match2_simp.dta"

merge 1:1 idstd using "D:\Research\WEBS\Geo_data\firm_city.dta"
drop _merge

drop if all_city ==""
duplicates drop join_fid, force

ren join_fid FID
encode all_city, gen (citycode)
gen city_id = citycode

keep FID all_city city_id
export delimited using "D:\Research\WEBS\Geo_data\city_definition.csv", replace
